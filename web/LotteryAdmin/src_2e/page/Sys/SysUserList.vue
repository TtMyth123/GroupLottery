  ...
  <div>
    <div class="container">
      ...
      <el-table
        ...
        <el-table-column prop="UserName" label="Tên thành viên" width="100"></el-table-column>
    ...
    <!-- 编辑弹出框 -->
    <el-dialog title="Chỉnh sửa của người quản lý " :visible.sync="editFormVisible" width="90%">
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        <el-form-item label="Tên thành viên" prop="UserName">
          <el-input v-model="form.UserName"></el-input>
        </el-form-item>
      </el-form>
      <span slot="footer" class="dialog-footer">
                <el-button @click="editFormVisible = false">Huỷ</el-button>
                <el-button type="primary" @click="saveEdit">Xác định</el-button>
            </span>
    </el-dialog>

    <!-- 编辑弹出框 -->
    <el-dialog title="Chỉnh sửa của đại lý" :visible.sync="editAgentFormVisible" width="90%">
      <el-form ref="form" :model="editAgentForm" :rules="rules" label-width="100px">
        <el-form-item label="Tên thành viên" prop="UserName">
          <el-input v-model="editAgentForm.UserName"></el-input>
        </el-form-item>
        <el-form-item label="ID của thành viên" prop="GameId">
          <el-input v-model="editAgentForm.GameId" readonly></el-input>
        </el-form-item>
      </el-form>
      <span slot="footer" class="dialog-footer">
                <el-button @click="editAgentFormVisible = false">Huỷ</el-button>
                <el-button type="primary" @click="saveAgentEdit">Xác định</el-button>
            </span>
    </el-dialog>

    <el-dialog title="Thêm đại lý" :visible.sync="addAgentFormVisible" width="300">
      <el-form ref="form" :model="formAddAgent" :rules="rules" label-width="100px">
        <el-form-item label="Tên thành viên" prop="UserName">
          <el-input v-model="formAddAgent.UserName"></el-input>
        </el-form-item>
        <el-form-item label="Mật khẩu" prop="Pwd">
      ...
      <span slot="footer" class="dialog-footer">
                <el-button @click="addAgentFormVisible = false">Huỷ</el-button>
                <el-button type="primary" @click="onAddAgentSysUser">Xác định</el-button>
            </span>
    </el-dialog>
    <el-dialog title="Thêm người quản lý" :visible.sync="addFormVisible" width="300">
      ...
      <span slot="footer" class="dialog-footer">
                <el-button @click="addFormVisible = false">Huỷ</el-button>
                <el-button type="primary" @click="onAddManageSysUser">Xác định</el-button>
            </span>
    </el-dialog>

    <el-dialog title="Quyền hạn cài đặt" :visible.sync="permissionFormVisible" width="90%">
      <el-table
          :data="tableDataMenu"
          border
          class="table"
          header-cell-class-name="table-header">
        <el-table-column prop="TypeName" label="Chia loại" width="100"></el-table-column>
        <el-table-column prop="Title" label="Công năng" width="180"></el-table-column>
        <el-table-column prop="Power" label="Chia loại" width="250">
          <template slot-scope="scope">
            <el-radio-group v-model="scope.row.Power">
              <el-radio :label="0">Cấm dùng</el-radio>
              <el-radio :label="1">Đọc</el-radio>
              <el-radio :label="2">Viết</el-radio>
            </el-radio-group>
          </template>
      ...
      <span slot="footer" class="dialog-footer">
                <el-button @click="permissionFormVisible = false">Huỷ</el-button>
                <el-button type="primary" @click="savePermission">Xác định</el-button>
            </span>
    </el-dialog>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        UserName: [
          { required: true, message: 'Mời nhập tên thành viên', trigger: 'blur' },
          { min: 1, max: 50, message: 'Từ2-50 chữ', trigger: 'blur' }
        ]
      }
...
// 富文本编辑器 内容改变事件
    ...
    handleDelete(index, row) {
      // 二次确认删除
      this.$confirm('Xác định xoá không？', 'Thông báo', {
        type: 'warning'
      })
          .then(() => {
            var data = {
            ...
            request({
              ...
              if (res.code == 200) {
                ...
                this.$message.error('Chưa xoá được：' + res.msg);
              }
            });
    ...
    onAddAgentSysUser() {
      request({
        ...
        if (res.code == 200) {
          ...
          this.$message.error('Chưa thêm thành công：' + res.msg);
        }
      });
    },
    onAddManageSysUser() {
      request({
        ...
        if (res.code == 200) {
          ...
          this.$message.error('Ch?a thêm thành c?ng：' + res.msg);
        }
      });
    },
    // 保存编辑
    saveEdit() {
      request({
        ...
        if (res.code == 200) {
          ...
          this.$message.success(`Chỉnh sửa thứ ${this.idx + 1} thành công`);
    ...
    saveAgentEdit() {
      request({
        ...
        if (res.code == 200) {
          this.editFormVisible = false;
          this.$message.success(`Chỉnh sửa thứ ${this.idx + 1} hành công`);
