...
func NewWsxNbcServer(gameType int, UserRpcClient *UserRpcClient.RpcClient) *WsxNbcServer {
	aWsxServer := new(WsxNbcServer)
...
func (this *WsxNbcServer) ReLoadOddsInfo() {
	this.OddsLock.Lock()
...
func (this *WsxNbcServer) GetRoomInfo() map[string]interface{} {
...
func (this *WsxNbcServer) RandNewAwardInfo() error {
	this.RandNewAwardInfoLock.Lock()
	defer this.RandNewAwardInfoLock.Unlock()

	aOpenCodeInfo := WsxBox.OpenCodeInfoNbc{}
	//391022
	aOpenCodeInfo.Jackpots = fmt.Sprintf("%06d", timeKit.GlobaRand.Intn(999999))
	//01738
	aOpenCodeInfo.FirstNum = fmt.Sprintf("%05d", timeKit.GlobaRand.Intn(99999))
	//41658
	aOpenCodeInfo.SecondNum = fmt.Sprintf("%05d", timeKit.GlobaRand.Intn(99999))
	//90148
	...
	//2961
	aOpenCodeInfo.FifthNum = fmt.Sprintf("%04d", timeKit.GlobaRand.Intn(9999))
	//057
	...
	//558
	aOpenCodeInfo.SeventhNum = fmt.Sprintf("%03d", timeKit.GlobaRand.Intn(999))
	//558
	aOpenCodeInfo.EighthNum = fmt.Sprintf("%02d", timeKit.GlobaRand.Intn(99))
...
func (this *WsxNbcServer) NewAwardInfo(newLoAwardInfo models.LoAwardInfo) {
	...
	for _, groupBet := range arrBetGroupInfo {
		...
		for _, bet := range mpBetInfo[groupBet.Id] {
			...
			win := WsxNbcResultKit.ComputeLoseWin(*bet, aOpenCodeInfo, this.mpOddsInfo)
...
type WsxAwardInfo1 struct {
	...
	WsxBox.OpenCodeInfoNbc
}

func (this *WsxNbcServer) Bet(betInfo LotteryBox.BetInfo) (map[string]interface{}, error) {
	this.OddsLock.RLock()
...
func (this *WsxNbcServer) GetHistoryResultList(PageIndex, PageSize, LastId int) (interface{}, error) {
	sqlWhereLastId := ""
	...
	arrWsxAwardInfo := make([]WsxAwardInfo1, len(arrData))
	for i := 0; i < len(arrData); i++ {
		...
		arrWsxAwardInfo[i].OpenCodeInfoNbc = aOpenCodeInfo
	}
...
func (this *WsxNbcServer) GetCurResult() (interface{}, error) {
	aWsxAwardInfo := WsxAwardInfo1{}
	...
	aWsxAwardInfo.OpenCodeInfoNbc = aOpenCodeInfo
