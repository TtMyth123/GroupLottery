...
func NewWsxResultServer1(resultUrl string, aAwardInfoFunc HandlerAwardInfoFunc) *WsxNbcResultServer {
	aWsxResultServer := new(WsxNbcResultServer)
...
func (this *WsxNbcResultServer) run() {
...
func (this *WsxNbcResultServer) saveAwardInfo() {
	...
	if this.CurOpenCodeInfo.NextIssue != aAwardInfo.NextIssue {
		aOpenCodeInfo := WsxBox.HttpOpenCodeNbc{}
		...
		aResultNums := WsxNbcResultKit.GetResultNums(aOpenCodeInfo)
...
func (this *WsxNbcResultServer) getAwardInfo() (*WsxBox.WsxResultData, error) {
