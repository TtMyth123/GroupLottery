...
func ComputeLoseWin(bet models.LoBetInfo, aOpenCodeInfo WsxBox.OpenCodeInfoNbc, mpLoOddsInfo map[int]models.LoOddsInfo) float64 {
	...
	case WsxConst.BigType_05, WsxConst.BigType_05B: //头特
		{
			strN := GetEndStr(aOpenCodeInfo.Jackpots, 2)
	...
	case WsxConst.BigType_06, WsxConst.BigType_06B: //尾特
		{
			strN := GetEndStr(aOpenCodeInfo.Jackpots, 1)
			if aLoOddsInfo.SNum == strN {
				win = strconvEx.Decimal(float64(bet.BetM) * bet.Odds)
	...
	case WsxConst.BigType_09, WsxConst.BigType_09B: //二等特码
		{
			strN := GetEndStr(aOpenCodeInfo.SecondNum, 2)
			if aLoOddsInfo.SNum == strN {
				win = strconvEx.Decimal(float64(bet.BetM) * bet.Odds)
				break
			}
	...
	case WsxConst.BigType_11: //头等 三连位
		{
			nums := make([]string, 0)
			nums = append(nums, aOpenCodeInfo.Jackpots)
			//nums = append(nums,aOpenCodeInfo.Jackpots,aOpenCodeInfo.FirstNum)
			//nums = append(nums,aOpenCodeInfo.SecondNum)
			//nums = append(nums,aOpenCodeInfo.ThirdNum...)
			//nums = append(nums,aOpenCodeInfo.ForthNum...)
	...
	case WsxConst.BigType_12, WsxConst.BigType_13: //平码两位区
		{
			...
			nums := make([]string, 0)
			nums = append(nums, aOpenCodeInfo.Jackpots, aOpenCodeInfo.FirstNum, aOpenCodeInfo.SecondNum,
				aOpenCodeInfo.FifthNum, aOpenCodeInfo.SeventhNum, aOpenCodeInfo.EighthNum)
			nums = append(nums)
			nums = append(nums, aOpenCodeInfo.ThirdNum...)
			nums = append(nums, aOpenCodeInfo.ForthNum...)
			nums = append(nums, aOpenCodeInfo.SixthNum...)
...
func GetResultNums(aOpenCodeInfo WsxBox.HttpOpenCodeNbc) map[string]interface{} {
